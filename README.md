...
Authorize Cloudflare to see your GitHub account (if you haven't already)
and pick the `sim-library` repo.
Build settings: this is a static site with no build step, so:
Build command: leave blank
Build output directory: `/` (the repo root)
Click Save and Deploy. Cloudflare will give you a URL like
`sim-library-abc.pages.dev` within a minute or two.
Optional: in the Pages project's Custom domains tab, you can rename
...
Create a new folder at the repo root (folder name becomes the URL slug,
e.g. `new-sim-name/`).
Put that simulation's `index.html` inside it.
Add a link to it in the root `index.html`, inside the right unit/topic
